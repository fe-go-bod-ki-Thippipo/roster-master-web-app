...
# X13: removing +1 must be detected even if all other exclusion keys and predicates remain intact.
run_mutation history_inclusive_end_x13 "ALTER TABLE public.employee_company_history DROP CONSTRAINT company_history_no_overlap; ALTER TABLE public.employee_company_history ADD CONSTRAINT company_history_no_overlap EXCLUDE USING gist (employee_id WITH =, daterange(effective_from,COALESCE(effective_to,'infinity'::date),'[)') WITH &&) WHERE (superseded_by_request_id IS NULL) DEFERRABLE INITIALLY DEFERRED;" "Company history exclusion supersede predicate or deferrability incorrect"
