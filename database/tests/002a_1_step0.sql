...
BEGIN
 ...
 IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid='employees'::regclass AND tgname='employee_code_immutable' AND tgenabled='O' AND NOT tgisinternal) THEN RAISE EXCEPTION 'Employee code immutability trigger missing'; END IF;
 IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid='audit_logs'::regclass AND tgname='audit_no_delete' AND tgenabled='O' AND NOT tgisinternal) THEN RAISE EXCEPTION 'Audit delete trigger missing'; END IF;
