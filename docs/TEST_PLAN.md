...
| EMP-08 | approved future transfer A->B, effective Nov 1 Asia/Bangkok; new compensating request approved Oct 20 | supersede original affected A/B versions and insert replacement A interval in one transaction; query as-of Nov 1 and inspect audit | only replacement effective interval counted; old versions and approval evidence retained; original superseded activation skipped; no DELETE/UPDATE original payload |
...
| INT-02 | employee + initial company history inserted in one transaction | commit with correct/mismatched home_company_id; query future transfer before effective date | deferred check accepts consistent projection and rejects mismatch; history authoritative for as-of reads |
| AUD-02 | approved future history with one-time supersede metadata | attempt second supersede, change original effective dates/provenance, delete original | all rejected; one-time supersede plus replacement succeeds atomically |

**Gate:** 002a-1 implementation PR must execute EMP-01..EMP-06, EMP-07..09, INT-01/02, AUD-01/02, MIG-02/03 and X12–X14/X16/X18 on disposable PostgreSQL and attach results. APR-13 midnight cancellation/activation race remains 002a-2; this documentation adds no SQL or executed tests.
