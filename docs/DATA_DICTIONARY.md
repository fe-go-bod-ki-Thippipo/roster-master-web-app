...
| employee_company_history / employees | Make effective-dated history authoritative; derive current home company or enforce atomic synchronization | Prevent overlapping or missing intervals as-of report date; migration must reconcile existing home_company_id |
...
| position_reporting_lines (NEW) | id uuid PK, position_id uuid FK, supervisor_position_id uuid FK, effective_from date, effective_to date?, approval_request_id uuid FK, status text | No self-reference, cycles or overlapping primary supervisors; cross-company edge only with approved request; policy on multiple supervisors pending |
| organization ownership history (NEW, exact table shape TBD) | Effective-dated position/department/division company lineage | Reconstruct position-owning company at historic as_of dates, prevent overlapping ownership, no silent backdated rewriting |
